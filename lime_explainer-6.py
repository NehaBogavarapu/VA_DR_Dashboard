...
def _explanation_to_heatmap(explanation, label, mask, original):
    """Convert LIME weights to a clean RGBA overlay.

    Generates a transparent PNG with ONLY the highlighted regions.
    The browser composites this over the original image.

    Colour scheme (high contrast against dark retinal images):
      Bright cyan (#00FFEE)  = positive contribution (model attends here)
      Hot yellow (#FFD600)   = negative contribution (artefact / misleading)
      White outlines         = region boundaries for clarity
    """
    h, w = mask.shape
    ...
    for seg_id, weight in local_exp.items():
        ...
        alpha = int(80 + 140 * intensity)  # 80–220 range
        ...
        # Draw white outline around region boundary
...
def _generate_synthetic_lime(img_array: np.ndarray) -> tuple:
    """Synthetic LIME-like overlay with high-visibility colours."""
    ...
    for i in range(n_regions):
        ...
        if rng.random() < 0.6:
            overlay[region] = [0, 255, 238, alpha]    # cyan = positive
        else:
            overlay[region] = [255, 214, 0, alpha]    # yellow = negative
...
def _draw_region_outline(overlay, region, color=(255, 255, 255, 200)):
    """Draw a thin outline around a binary region in the RGBA overlay."""
