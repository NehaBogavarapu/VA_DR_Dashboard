...
def get_learner():
    ...
    # export.pkl pickled a reference to quadratic_kappa from the notebook's
    # __main__ module. We must inject it into __main__ before unpickling.
    def quadratic_kappa(y_hat, y):
        return torch.tensor(
            cohen_kappa_score(torch.round(y_hat), y, weights='quadratic'),
            device='cpu',
        )
    ...
    if main_module is not None:
        main_module.quadratic_kappa = quadratic_kappa
