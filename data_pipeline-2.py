...
def get_umap_embeddings(
    ...
    """Project the filtered subset of embeddings into 2-D via UMAP.

    Looks up embedding rows by matching image_id order between the
    predictions CSV and the embeddings array (same order guaranteed
    by the notebook export).
    ...
    # Map image_id → row index in the full dataset
    ...
    # Cache key
    cache_key = (tuple(sorted(indices)), n_neighbors, min_dist)
    ...
    reducer = umap.UMAP(
        n_components=2,
        n_neighbors=min(n_neighbors, len(scaled) - 1),  # guard against small subsets
        min_dist=min_dist,
        metric="cosine",
